...
public class BookingService {
    ...
    static {
        ...
        BOOKINGS.put("123-456", new Booking("123-456", LocalDate.now().plusDays(1), LocalDate.now().plusDays(7), new Customer("James", "Bond"), false, "Aston Martin")); // Not cancelable: too late
        BOOKINGS.put("234-567", new Booking("234-567", LocalDate.now().plusDays(10), LocalDate.now().plusDays(12), new Customer("James", "Bond"), false, "Renault")); // Not cancelable: too short
        BOOKINGS.put("345-678", new Booking("345-678", LocalDate.now().plusDays(14), LocalDate.now().plusDays(20), new Customer("James", "Bond"), false, "Porsche")); // Cancelable
        // Emilio Largo: villain frauder!
        BOOKINGS.put("456-789", new Booking("456-789", LocalDate.now().plusDays(10), LocalDate.now().plusDays(20), new Customer("Emilio", "Largo"), false, "Porsche")); // Cancelable
        BOOKINGS.put("567-890", new Booking("567-890", LocalDate.now().plusDays(11), LocalDate.now().plusDays(16), new Customer("Emilio", "Largo"), false, "BMW")); // Cancelable
    }
    ...
    private Booking checkBookingExists(String bookingNumber, String name, String surname) {
        ...
        if (booking == null || !booking.getCustomer().getName().equals(name) || !booking.getCustomer().getSurname().equals(surname)) {
    ...
    public Booking getBookingDetails(
            @ToolArg(description = "The booking id composed of three digits followed by a minus then three digits") String bookingNumber,
    ...
    public List<String> getBookingsForCustomer(
        ...
        return BOOKINGS.values().stream().filter(booking -> booking.getCustomer().equals(customer)).map(Booking::getBookingNumber).collect(Collectors.toList());
    }
